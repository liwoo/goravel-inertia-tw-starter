...
import { Bdsp, BdspUpdateData, BdspService } from '@/types/bdsp';
import { User, MapPin, Tag, FileText, Plus, X } from 'lucide-react';
...
interface BdspEditFormProps extends CrudEditFormProps<Bdsp> {
  setIsSaving?: (saving: boolean) => void;
}

export const BdspEditForm = forwardRef<any, BdspEditFormProps>(({
  item: bdsp,
  onSuccess,
  onError,
  onCancel,
  ...
  const [formData, setFormData] = useState<BdspUpdateData>({
    name: bdsp.name,
    postal_address: bdsp.postal_address,
    physical_address: bdsp.physical_address || '',
    registration_status: bdsp.registration_status || '',
    partners: bdsp.partners || [],
    product_types: bdsp.product_types || [],
    service_list: bdsp.service_list || [],
  });
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.service_list || formData.service_list.length === 0) newErrors.serviceList = 'At least one service is required';
    ...
    try {
      const response = await fetch(`/api/bdsps/${bdsp.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        onSuccess('Bdsp updated successfully');
      } else {
  ...
  return (
    <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Bdsp Information</CardTitle>
            <CardDescription>Update BDSP details</CardDescription>
          </CardHeader>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {formData.service_list && formData.service_list.length > 0 && (
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="font-medium text-foreground">#{bdsp.id}</p>
            </div>
            <div>
              <p className="text-muted-foreground">Created</p>
              <p className="font-medium text-foreground">
                {(bdsp.createdAt || bdsp.created_at) ? new Date(bdsp.createdAt || bdsp.created_at || '').toLocaleDateString() : '-'}
              </p>
            </div>
            <div>
              <p className="text-muted-foreground">Last Updated</p>
              <p className="font-medium text-foreground">
                {(bdsp.updatedAt || bdsp.updated_at) ? new Date(bdsp.updatedAt || bdsp.updated_at || '').toLocaleDateString() : '-'}
              </p>
            </div>
          </div>
        </div>
...
BdspEditForm.displayName = 'BdspEditForm';
