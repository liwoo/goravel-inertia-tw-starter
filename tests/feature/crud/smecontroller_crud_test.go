...
func (s *SmeControllerCRUDTestSuite) TestPagination() {
	...
	for i := 1; i <= 25; i++ {
		sme := &models.Sme{
			...
			BusinessCategory:           "Retail",
			Sector:                     "Commerce",
			ContactPhone:               fmt.Sprintf("+26599900%04d", i),
...
func (s *SmeControllerCRUDTestSuite) TestSorting() {
	...
	for i, name := range names {
		sme := &models.Sme{
			...
			BusinessCategory:           "Services",
			Sector:                     "Technology",
			ContactPhone:               fmt.Sprintf("+26599910%04d", i),
...
func (s *SmeControllerCRUDTestSuite) TestListSmes() {
	...
	for i := 1; i <= 3; i++ {
		sme := &models.Sme{
			...
			BusinessCategory:           "Services",
			Sector:                     "Consulting",
			ContactPhone:               fmt.Sprintf("+26599920%04d", i),
...
func (s *SmeControllerCRUDTestSuite) TestUBISequentialNumbers() {
	...
	for i := 0; i < 3; i++ {
		smeData := map[string]interface{}{
			...
			"business_category":            "Micro",
			"sector":                       "Services",
			"contact_phone":                fmt.Sprintf("+26599SEQ%d", i),
...
func (s *SmeControllerCRUDTestSuite) TestUBIUniqueness() {
	...
	for i := 0; i < 5; i++ {
		smeData := map[string]interface{}{
			...
			"contact_phone":                fmt.Sprintf("+26599UNQ%d", i),
