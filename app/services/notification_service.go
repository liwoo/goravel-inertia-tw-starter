...
// NotifyApplicationStatusSync sends notification and message to a user when their application is approved/rejected
...
func (s *NotificationService) NotifyApplicationStatusSync(
	...
	if status == "approved" {
		...
		message = fmt.Sprintf("Your application for %s has been approved. Welcome!", smeName)
		messageContent = fmt.Sprintf("✅ **Application Approved**\n\nCongratulations! Your application for **%s** has been approved.\n\nYou can now access all the features available in the portal.", smeName)
	} else { // rejected
		priority = "high"
